...
states.each do |place, abbreviation|
  p "I have visited #{place}, this is it's abbreviation #{abbreviation}"
end
...
birthdays.each do |person, date|
  p "My friend #{person}, has a birthday on #{date}"
end
...
login_statuses.each do |person, status|
  if status == true
    puts person
  end
end
